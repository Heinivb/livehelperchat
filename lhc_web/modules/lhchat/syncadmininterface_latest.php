...
if ($activeTabEnabled == true) {
    ...
    $addChats  = array();
	foreach ($chats as $kk => $chat)
	{  
        $json = json_decode($chat->additional_data);
        $addChats[$kk] = $json;
 ...
 //print_r($chats);die;
	 erLhcoreClassChat::prefillGetAttributes($chats,array('time_created_front','department_name','plain_user_name','product_name','n_official','n_off_full','additional_data'),array('product_id','product','department','time','status','user_id','user','additional_data','additional_data_array','chat_variables','chat_variables_array'),array('additional_columns' => $columnsAdditional));


     foreach ($chats as $kk => $chat)    {  
        
        $chats[$kk]->additional_data = $addChats[$kk];
    }
...
if ($myChatsEnabled == true) {
    ...
    foreach ($myChats as $kk => $chat)    {  
        ...
        $myChats[$kk]->additional_data = $addChats[$kk];
    }
...
if ($pendingTabEnabled == true) {
   ...
   $addChats = array();
    foreach ($pendingChats as $kk => $chat)
    {  
        $json = json_decode($chat->additional_data);
        $addChats[$kk] = $json;
    }
    ...
    foreach ($pendingChats as $kk => $chat)    {  
        ...
        $pendingChats[$kk]->additional_data = $addChats[$kk];
    }
