<table class="table table-sm mb-0 table-small table-fixed" ng-if="pending_chats.list.length > 0">
    <thead>
    <tr>
        <th width="48%"><i title="<?php echo erTranslationClassLhTranslation::getInstance()->getTranslation('chat/syncadmininterface','Visitor')?>" class="material-icons">face</i><a ng-click="lhc.toggleWidget('pending_chats_sort',true)"><i title="<?php echo erTranslationClassLhTranslation::getInstance()->getTranslation('chat/syncadmininterface','Sort')?>" class="material-icons">{{lhc.toggleWidgetData['pending_chats_sort'] == false ? 'trending_up' : 'trending_down'}}</i></a></th>
    ...
    <tr ng-repeat="chat in pending_chats.list track by chat.id" ng-class="{'user-away-row': chat.user_status_front == 2, 'user-online-row': chat.user_status_front == 0}">
       ...
       <!-- <td>
            <div class="abbr-list" title="{{chat.additional_data[0].value}}">
                
                           {{chat.additional_data[0].value}}
                        </div>
                
       </td>
       <td>
            <a target="_blank" href="https://www.myaie.ac/cadcocms/intakes_list_edit.php?uid={{chat.additional_data[3].value}}">{{chat.additional_data[2].value}} </a>
