...
<?php
    ...
    if (empty($UID)) 
    {
                  $api = "https://www.crm.aie.ac";  

                  if( $_SERVER['HTTP_HOST'] =='www.livehelper.conceptinteractive.co.za' ) {

                           $api = 'http://crm1.myway.training';
                  }

                  $url = $api."/get_leadsByemail.php?authCRM=crmAdmin001&email=".$chat->email;               
                  ...
                  if ( count($detail->results->leads) || count($detail->results->accounts_by_contact) || count($detail->results->contacts) || count($detail->results->opportunity_by_contact) ) {
