...
preloadWidget : function() {
      ...
      var userD = username +'_'+email+'_'+userID; 
...
showStartWindow : function(url_to_open,delayShow) {
    ...
    if (this.isProactivePending == 1 && this.invitationUrl != ''){
      ...
      if ( url_to_open != undefined ) {
            ...
            this.initial_iframe_url = url_to_open+this.getAppendCookieArguments()+'?userD='+userD+'&URLReferer='+locationCurrent+this.parseOptions()+this.parseStorageArguments()+'&dt='+encodeURIComponent(document.title);
            this.iframeCustomUrl = true;
      } else {
      		this.chatOpenedCallback(this.isOnline == false ? 'internal_offline' : 'internal');	
            this.initial_iframe_url = "<?php echo erLhcoreClassModelChatConfig::fetch('explicit_http_mode')->current_value?>//<?php echo $_SERVER['HTTP_HOST']?><?php echo erLhcoreClassDesign::baseurlsite()?>"+this.lang+"/chat/chatwidget<?php $leaveamessage == true ? print '/(leaveamessage)/true' : ''?><?= isset($currentPosition['full_height']) && $currentPosition['full_height'] ?  '/(fullheight)/true' : '/(fullheight)/false' ?><?php $department !== false ? print '/(department)/'.$department : ''?><?php $bot_id !== null ? print '/(bot_id)/'.$bot_id : ''?><?php $theme !== false ? print '/(theme)/'.$theme->id : ''?><?php $operator !== false ? print '/(operator)/'.$operator : ''?><?php $priority !== false ? print '/(priority)/'.$priority : ''?><?php $uarguments !== false ? print '/(ua)/'.$uarguments : '' ?>"+this.getAppendCookieArguments()+'?userD='+userD+'&URLReferer='+locationCurrent+this.parseOptions()+this.parseStorageArguments()+'&dt='+encodeURIComponent(document.title);
      };
    ...
    var lhc_obj = this;
      ...
      document.getElementById('<?php echo $chatCSSPrefix?>_min').onclick = function() { lhc_obj.min(); return false; };
       document.getElementById('<?php echo $chatCSSPrefix?>_online_icon').onclick = function() { lhc_obj.min(); return false; };
